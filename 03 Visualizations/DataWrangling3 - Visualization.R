VIS3 <- DATA3 %>% ggplot(aes(x = UNEMPLOYMENT_RATE_AVG, y = INTEREST_RATE_AVG))+
  geom_point(aes(color=STATE))+ geom_text(aes(label=STATE, hjust=1.5), size=3)
...
#Basic Formatting
VIS3 <- VIS3 + theme(panel.background = element_rect(fill = 'grey92',color = 'grey1')) + theme(legend.position='none')
